...
function initializeLegendControls() {
    ...
    // Re-query after cloning to get new elements
    document.querySelectorAll('.legend-item.clickable').forEach(item => {
        item.addEventListener('click', function() {
            ...
            this.style.opacity = this.dataset.active === 'true' ? '1' : '0.5';
            
            // Show/hide markers of this status
            if (typeof stationMarkers !== 'undefined' && stationMarkers.length > 0) {
                stationMarkers.forEach((markerArray, index) => {
                    if (Array.isArray(markerArray) && typeof currentStations !== 'undefined') {
                        const station = currentStations[index];
                        if (station) {
                            const stationStatus = typeof determineStationStatus === 'function' 
                                ? determineStationStatus(station) 
                                : station.stationStatusCode;
...
// Initialize station status filters (alias for backward compatibility)
