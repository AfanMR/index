...
function initializeControls() {
    // Initialize legend controls only once
    ...
    if (periodTypeSelect) {
        periodTypeSelect.addEventListener('change', function() {
            ...
            // Update legend for any monitoring category (to update colors)
    ...
    // Variable type selector - removed (not used in upper air API)
    /*
    ...
    */
    ...
    if (monitoringCategorySelect) {
        monitoringCategorySelect.addEventListener('change', function() {
            ...
            const variableControl = document.getElementById('variableControl');
            
            // Show variable dropdown only for quality monitoring
            if (variableControl) {
                variableControl.style.display = category === 'quality' ? 'block' : 'none';
            }
    ...
    if (baselineSelect) {
        baselineSelect.addEventListener('change', function() {
            // Update legend colors when baseline changes
            updateLegendForMonitoringCategory();
            loadStationData();
        });
    }
...
async function checkApiAccess() {
    try {
        ...
        const variable = 'pressure'; // Default variable for upper air
        const monitoringCategory = document.getElementById('monitoringCategory').value;
        ...
        const response = await fetch(`${API_ENDPOINT}upper_air_nwp.php?territory=${territory}&date=${date}&period=${period}&time_period=${time_period}&variable=${variable}&monitoring_category=${monitoringCategory}&centers=${monitoringCentre}&baseline=${baseline}`);
        return response.ok;
    } catch (error) {
...
function updateLegendForMonitoringCategory() {
    ...
    if (monitoringCategory === 'timeliness') {
        ...
        if (periodType === 'monthly') {
            ...
            observationTitle.textContent = 'Received soundings';
        }
        
        // Remove quality mode class
        document.querySelector('.legend').classList.remove('quality-mode');
        
        // Update legend colors based on period type and baseline
        if (periodType === 'daily') {
            // Daily period: Use new color scheme from image
            // Show more-than-100 item for pink color
            const moreThan100Item = document.querySelector('.legend-item[data-status="more-than-100"]');
            if (moreThan100Item) {
                moreThan100Item.style.display = 'flex';
            }
            updateLegendItem('more-than-100', '#e91e63', 'More than declared in OSCAR/Surface');
            updateLegendItem('operational', '#22c55e', 'No issue');
            updateLegendItem('issues', '#f97316', 'Completeness issue');
            updateLegendItem('critical', '#dc2626', 'Availability issue');
            updateLegendItem('not-received', '#000000', 'Not received in period');
            updateLegendItem('no-match', '#eab308', 'No match in OSCAR/Surface');
        } else {
            // Six-hour, monthly, or other: Use default color scheme
            // Hide more-than-100 item for non-daily periods
            const moreThan100Item = document.querySelector('.legend-item[data-status="more-than-100"]');
            if (moreThan100Item) {
                moreThan100Item.style.display = 'none';
            }
            updateLegendItem('operational', '#22c55e', 'At least one complete launch (all variables and layers)');
            updateLegendItem('issues', '#f97316', 'Incomplete launch (missing variables)');
            updateLegendItem('critical', '#dc2626', 'Incomplete launch (missing layers)');
            updateLegendItem('not-received', '#000000', 'Not received in period');
            updateLegendItem('no-match', '#eab308', 'No match in OSCAR/Surface');
        }
        ...
        allLegendItems.forEach(item => {
            if (item.classList.contains('quality-only') || item.classList.contains('timeliness-only')) {
                item.style.display = 'none';
            } else if (item.getAttribute('data-status') === 'more-than-100' && periodType !== 'daily') {
                // Keep more-than-100 hidden for non-daily periods
                item.style.display = 'none';
            } else {
                item.style.display = 'flex';
            }
...
// Helper function to update legend item color and text
function updateLegendItem(status, color, text) {
    const item = document.querySelector(`.legend-item[data-status="${status}"]`);
    if (item) {
...
async function loadStationData() {
    ...
    const monitoringCategory = document.getElementById('monitoringCategory').value;
    const variableSelect = document.getElementById('variableType');
    const variable = (monitoringCategory === 'quality' && variableSelect) ? variableSelect.value : 'pressure';
    const date = document.getElementById('observationDate').value;
    const periodType = document.getElementById('periodType').value;
    ...
    try {
        ...
        if (periodType === 'monthly') {
            ...
            apiUrl = `${API_ENDPOINT}upper_air_nwp.php?territory=${territory}&variable=${variable}&date=${monthDate}&period=${apiPeriodType}&monitoring_category=${monitoringCategory}&centers=${monitoringCentre}&baseline=${baseline}`;
        } else {
            // Build API URL - exclude baseline parameter for quality monitoring
            if (monitoringCategory === 'quality') {
                apiUrl = `${API_ENDPOINT}upper_air_nwp.php?territory=${territory}&variable=${variable}&date=${date}&period=${apiPeriodType}&monitoring_category=${monitoringCategory}&centers=${monitoringCentre}`;
            } else {
                apiUrl = `${API_ENDPOINT}upper_air_nwp.php?territory=${territory}&variable=${variable}&date=${date}&period=${apiPeriodType}&monitoring_category=${monitoringCategory}&centers=${monitoringCentre}&baseline=${baseline}`;
            }
...
function updateStationStatus() {
    ...
    stationMarkers.forEach((marker, index) => {
        ...
        if (station) {
            // Determine status based on monitoring category
            let newStatus;
            if (monitoringCategory === 'quality' || monitoringCategory === 'timeliness') {
                // Recalculate status using MINIMUM logic (quality or timeliness)
                newStatus = determineStationStatus(station);
            } else {
                // Use status from API for availability monitoring
                newStatus = station.stationStatusCode || 'not-received';
            }
    ...
    stationMarkers.forEach(markerArray => {
        if (Array.isArray(markerArray)) {
            ...
            if (station && markerArray[0] && map.hasLayer(markerArray[0])) {
                ...
                // Get actual status based on monitoring category
                let status;
                if (monitoringCategory === 'quality' || monitoringCategory === 'timeliness') {
                    // Recalculate status for quality and timeliness
                    status = determineStationStatus(station);
                } else {
                    // Use status from API for availability
                    status = station.stationStatusCode || 'not-received';
                }
...
function determineStationStatus(station) {
    ...
    if (monitoringCategory === 'timeliness') {
        ...
        // Step 2: Calculate total coverage from all centers
        let totalReceived = 0;
        let totalExpected = 0;
        ...
        centers.forEach(center => {
            if (station.centers && station.centers[center]) {
                ...
                totalExpected += station.centers[center].expected || 0;
            }
        });
        ...
        // Step 3: Determine status based on total coverage
        if (totalCoverage >= 80) {
            return 'operational'; // Green - Normal (≥ 80%)
        } else if (totalCoverage >= 30) {
            return 'issues';      // Orange - Availability Issues (≥ 30%)
        } else if (totalCoverage > 0) {
            return 'critical';    // Red - Availability Issues (< 30%)
        } else {
            return 'not_received'; // Black - Not received in period
        }
    }
}
...
function createStationMarker(station, status) {
    ...
    if (monitoringCategory === 'timeliness') {
        ...
        // Availability monitoring colors - change based on period and baseline
        const periodType = document.getElementById('periodType').value;
        const baseline = document.getElementById('baseline').value;
        
        if (periodType === 'daily' && baseline === 'OSCAR') {
            // Daily + OSCAR: Use new color scheme
            colors = {
                'more-than-100': '#e91e63',   // pink - More than declared in OSCAR/Surface
                'operational': '#22c55e',     // green - No issue
                'issues': '#f97316',          // orange - Completeness issue
                'critical': '#000000',        // black - Not received in period
                'not-received': '#000000',    // black - Not received in period
                'oscar-issue': '#9ca3af',     // gray - OSCAR schedule issue
                'no-match': '#eab308'         // yellow - No match in OSCAR/Surface
            };
        } else {
            ...
            colors = {
                'more-than-100': '#e91e63',   // pink - More than 100%
                'operational': '#22c55e',     // green - At least one complete launch
                'issues': '#f97316',          // orange - Incomplete launch (missing variables)
                'critical': '#dc2626',        // red - Incomplete launch (missing layers)
                'not-received': '#000000',    // black - Not received in period
                'oscar-issue': '#9ca3af',     // gray - OSCAR schedule issue
                'no-match': '#eab308'         // yellow - No match in OSCAR/Surface
            };
...
function displayStations(stations) {
    try {
        ...
        stations.forEach(station => {
            // For quality and timeliness monitoring, recalculate status based on MINIMUM value
            // For availability monitoring, use status from API
            const monitoringCategory = document.getElementById('monitoringCategory').value;
            let status;
