...
try {
    ...
    // Check if we're requesting USA stations (CHECK BEFORE modifying $territory)
    ...
    // Check territories list for USA codes
    $territories = explode(',', $territory);
    $territories_usa = ['LIH', 'HNL', 'OGG', 'ITO', 'GUM', 'PPG']; // USA Pacific territory codes
    ...
    // Skip baseline validation for quality monitoring
    if ($monitoring_category !== 'quality' && !in_array($baseline, $valid_baselines)) {
        $baseline = 'OSCAR'; // Default to OSCAR if invalid
    }
    ...
    // Log file setup
    $log_message = date('Y-m-d H:i:s') . " - NWP Request: $territory | $monitoring_category | $period\n";
    ...
    if ($is_usa_territory) {
        // Build USA API URL - exclude baseline for quality and timeliness monitoring
        // NOTE: Fetch all global data and filter by station name in PHP (countries parameter causes issues)
        if ($monitoring_category === 'quality' || $monitoring_category === 'timeliness') {
            $usa_api_url = "https://wdqms.wmo.int/wdqmsapi/v1/download/nwp/synop/$period/$monitoring_category/?" . 
                           "date=$date&period=$time_period&variable=$variable&centers=$centers";
        } else {
            $usa_api_url = "https://wdqms.wmo.int/wdqmsapi/v1/download/nwp/synop/$period/$monitoring_category/?" . 
                           "date=$date&period=$time_period&variable=$variable&centers=$centers&baseline=$baseline";
        }
        ...
        // Log USA response only if error
        if ($error || $httpCode != 200) {
            $log_message = date('Y-m-d H:i:s') . " - USA API Error: HTTP $httpCode" . ($error ? " - $error" : "") . "\n";
            file_put_contents($log_file, $log_message, FILE_APPEND);
        }
        ...
        if (!$error && $httpCode == 200) {
            ...
            if (count($lines) > 1) {
                ...
                foreach ($usaStationDataByWigosId as $wigosId => $stationData) {
                    // USA stations are already filtered by target_stations above, so always process them
                ...
                // Continue to fetch Region V data (SGP,IDN,BRN,PHL,TLS,PNG,MYS)
            }
        }
    }
    ...
    // Build API URL - exclude baseline for quality and timeliness monitoring
    if ($monitoring_category === 'quality' || $monitoring_category === 'timeliness') {
        $url = "https://wdqms.wmo.int/wdqmsapi/v1/download/nwp/synop/$period/$monitoring_category/?" . 
               "date=$date&period=$time_period&variable=$variable&centers=$centers";
    } else {
        $url = "https://wdqms.wmo.int/wdqmsapi/v1/download/nwp/synop/$period/$monitoring_category/?" . 
               "date=$date&period=$time_period&variable=$variable&centers=$centers&baseline=$baseline";
    ...
    // Log only if error
    if ($error || $httpCode != 200) {
        $log_message = date('Y-m-d H:i:s') . " - Region V API Error: HTTP $httpCode" . ($error ? " - $error" : "") . "\n";
        file_put_contents($log_file, $log_message, FILE_APPEND);
    }
    ...
    if ($error) {
        ...
        // Skip JSON logging for performance
                ...
                for ($i = 1; $i < count($lines); $i++) {
                    if (!empty(trim($lines[$i]))) {
                        ...
                        if (count($row) >= count($headers)) {
                            ...
                            // FILTER: Only process stations from requested territories (like GBON)
                            // Check both 'country code' (with space) and 'country' field names
                            $countryCode = '';
                            if (isset($stationData['country code'])) {
                                $countryCode = strtoupper(trim($stationData['country code']));
                            } elseif (isset($stationData['country'])) {
                                $countryCode = strtoupper(trim($stationData['country']));
                            }
                            
                            if (!in_array($countryCode, $territories)) {
                                continue; // Skip stations not in SGP,IDN,BRN,PHL,TLS,PNG,MYS
                            }
        ...
        // Combine USA stations with Region V stations if both were fetched
        ...
        // Log final results
        $log_message = date('Y-m-d H:i:s') . " - Result: $total stations (OK:$operational, Iss:$issues, Crit:$critical, NR:$not_received)\n";
        file_put_contents($log_file, $log_message, FILE_APPEND);
        ...
        // Log error
        $log_message = date('Y-m-d H:i:s') . " - Error: $apiStatus\n";
        file_put_contents($log_file, $log_message, FILE_APPEND);
