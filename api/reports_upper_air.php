...
try {
    ...
    $variable = isset($_GET['variable']) ? strtolower($_GET['variable']) : 'temperature';
    ...
    if (!in_array($variable, $valid_variables)) {
        $variable = 'temperature'; // Default to temperature if invalid
    }
    ...
    if ($is_usa_territory) {
        $usa_api_url = "https://wdqms.wmo.int/wdqmsapi/v1/download/gbon/temp/$period/availability/?" . 
                       "date=$date&variable=$variable&centers=DWD,ECMWF,JMA,NCEP&region=pacific";
        
        $log_message .= "USA API URL: $usa_api_url\n";
        $log_message .= "Filtering USA Pacific stations\n";
            ...
            if (count($lines) > 1) {
                ...
                foreach ($stationDataByWigosId as $wigosId => $stationData) {
                    if (in_array($stationData['territoryCode'], $territories) || $fetch_usa_territories) {
                        ...
                        foreach ($stationData['centers'] as $center => $centerData) {
                            if ($centerData['expected'] > 0) {
                                $center_coverages[$center] = [
                                    'received' => $centerData['received'],
                                    'expected' => $centerData['expected'],
                                    'coverage' => ($centerData['received'] / $centerData['expected']) * 100
                                ];
                            }
                        }
                        
                        // Determine overall status and color code
                        $status = 'not_received';
                        $color_code = 'black';
                        
                        // Special case: If NCEP is low but JMA & ECMWF have good coverage (≥80%) and DWD has data
                        if (isset($center_coverages['NCEP']) && 
                            $center_coverages['NCEP']['coverage'] < 80 && 
                            $center_coverages['NCEP']['coverage'] > 0 &&
                            isset($center_coverages['JMA']) && $center_coverages['JMA']['coverage'] >= 80 &&
                            isset($center_coverages['ECMWF']) && $center_coverages['ECMWF']['coverage'] >= 80 &&
                            isset($center_coverages['DWD']) && $center_coverages['DWD']['received'] > 0) {
                            $status = 'complete';
                            $color_code = 'green';
                        }
                        // For monthly period: Check if NCEP is the primary determinant
                        else if (isset($center_coverages['NCEP'])) {
                            $ncep_coverage = $center_coverages['NCEP']['coverage'];
                            
                            if ($ncep_coverage >= 80) {
                                $status = 'complete';
                                $color_code = 'green';
                            } else if ($ncep_coverage >= 30) {
                                $status = 'issues';
                                $color_code = 'orange';
                            } else if ($ncep_coverage > 0) {
                                // Check if any center has very low reception (below 20%)
                                $has_very_low_reception = false;
                                foreach ($center_coverages as $center => $coverage) {
                                    if ($coverage['received'] > 0 && $coverage['coverage'] < 20) {
                                        $has_very_low_reception = true;
                                        break;
                                    }
                                }
                        ...
                        $summary = $stationData['date'] . '_' . $stationData['countryCode'] . '_' . 
                                  ...
                                  $statusDescription . '_GBON Availability Upper-air';
                ...
                if (!$fetch_usa_territories && $is_usa_territory) {
                    echo json_encode([
                        ...
                        'metadata' => [
                            ...
                            'complete' => count(array_filter($usa_stations, function($s) { return $s['stationStatusCode'] === 'complete'; })),
    ...
    // Build WDQMS API URL (without countries parameter - filter from response instead)
    $url = "https://wdqms.wmo.int/wdqmsapi/v1/download/gbon/temp/$period/availability/?" . 
           "date=$date&variable=$variable&centers=DWD,ECMWF,JMA,NCEP";

    $log_message .= "API URL: $url\n";
    $log_message .= "Filtering countries: $territory\n";
    file_put_contents($log_file, $log_message, FILE_APPEND);
    ...
    if ($error) {
                ...
                for ($i = 1; $i < count($lines); $i++) {
                    if (!empty(trim($lines[$i]))) {
                        ...
                        if (count($row) >= count($headers)) {
                            ...
                            // Filter by country code - only process if country is in our territories list
                            $countryCode = $stationData['country code'] ?? '';
                            ...
                            // Initialize station data if first time seeing this station
                            if (!isset($stationDataByWigosId[$wigosId])) {
                                $actualCountryCode = $stationData['country code'] ?? '';
                                $stationDataByWigosId[$wigosId] = [
                                    ...
                                    'territory' => $actualCountryCode,
                                    'territoryCode' => $actualCountryCode,
                                    'stationTypeName' => 'SYNOP',
        ...
        // Combine USA stations with regional stations if fetching both
        if ($fetch_usa_territories && !empty($usa_stations)) {
            $stations = array_merge($stations, $usa_stations);
        }
