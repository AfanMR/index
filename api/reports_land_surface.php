...
try {
    ...
    if ($is_usa_territory) {
        $usa_api_url = "https://wdqms.wmo.int/wdqmsapi/v1/download/gbon/synop/$period/availability/?" . 
                       "date=$date&period=$time_period&variable=$variable&centers=DWD,ECMWF,JMA,NCEP&region=pacific";
                ...
                foreach ($stationDataByWigosId as $wigosId => $stationData) {
                    if (in_array($stationData['territoryCode'], $territories) || $fetch_usa_territories) {
                        ...
                        $summary = $stationData['date'] . '_' . $stationData['countryCode'] . '_' . 
                                  ...
                                  $statusDescription . '_GBON Availability Surfaceland';
                ...
                if (!$fetch_usa_territories && $is_usa_territory) {
                    echo json_encode([
                        ...
                        'metadata' => [
                            ...
                            'operational_count' => count(array_filter($usa_stations, function($s) { return $s['stationStatusCode'] === 'complete'; })),
    ...
    $url = "https://wdqms.wmo.int/wdqmsapi/v1/download/gbon/synop/$period/availability/?" . 
           "date=$date&period=$time_period&variable=$variable&centers=DWD,ECMWF,JMA,NCEP";
    ...
    if ($error) {
                ...
                foreach ($stationDataByWigosId as $wigosId => $stationData) {
                    ...
                    switch ($status) {
                        ...
                        case 'critical':
                            ...
                            break;
                        case 'not_received':
                            $statusDescription = 'Not_Received';
                            break;
                    }
        ...
        // Merge USA stations with Region V stations if we have both
