...
<main class="container mx-auto px-4 py-8 lg:py-12 max-w-7xl">
    ...
    <div class="bg-white rounded-lg shadow-md p-4 lg:p-6 mb-6">
        ...
        <div class="flex flex-col gap-6 mb-8">
            <!-- Station Availability Bar Chart -->
            <div class="bg-gray-50 rounded-lg p-4 lg:p-6 relative">
                <div class="text-center text-lg font-semibold text-gray-700 mb-4">Station Availability Overview (Monthly)</div>
                <div class="relative w-full overflow-x-auto">
                    <div style="height: 400px; min-height: 300px; min-width: 600px;">
...
<script>
    ...
    function populateStationSelector(stations) {
        ...
        // Reset selection on new data load (unless we want to persist?)
        // Let's reset to ALL if the list of stations significantly changed.
        ...
        newSelectAll.checked = true; // Reset
    ...
    function initializeCharts() {
        ...
        allStationsChart = new Chart(allStationsCtx.getContext('2d'), {
            ...
            options: {
                ...
                plugins: {
                    legend: {
                        display: false // No legend needed for single dataset color-coded
                    },
                    tooltip: {
                        callbacks: {
    ...
    function updateCharts(stations) {
        ...
        setTimeout(() => {
            try {
                ...
                // 2. Update All Stations Availability Chart (Bar)
                if (allStationsChart) {
                    // Prep Data
                    const labels = stations.map(s => s.wigosId || s.name);
                    const values = stations.map(s => {
                       if (s.dataCompleteness !== undefined) return s.dataCompleteness;
                       ...
                       if (s.centers) {
                           CENTERS.forEach(c => {
                               if(s.centers[c]) {
                                   totalR += s.centers[c].received || 0;
                                   totalE += s.centers[c].expected || 6;
                               }
                           });
                       }
                       return totalE > 0 ? Math.round((totalR / totalE) * 100) : 0;
                    });
                    
                    // Resize container logic
                    const chartCanvas = document.getElementById('allStationsChart');
                    const container = chartCanvas.parentElement; // div with style height/width
                    
                    // Min width 30px per bar or 600px total
                    const minWidthPerBar = 30;
                    const newWidth = Math.max(600, labels.length * minWidthPerBar);
                    
                    // Apply width to the container inside the overflow-x-auto
                    if (container) {
                        container.style.width = `${newWidth}px`;
                    }
                    
                    allStationsChart.resize(); // Trigger resize
                    
                    allStationsChart.data.labels = labels;
                    allStationsChart.data.datasets[0].data = values;
